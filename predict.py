...
class MolPredict(object):
    ...
    def __init__(self, load_model=None, cache_dir= None, extract_feature= False):
        """ 
        ...
        self.config.cache_dir_test= cache_dir
    ...
    def predict(self, data, save_path=None, metrics='none'):
        ...
        if not self.extract_feature:
            ...
            self.model = NNModel(self.datahub.data, self.datahub.data, self.trainer, **self.config)
            ...
            self.trainer = Trainer(save_path=self.load_model, **self.config)
            self.model = NNModel(self.datahub.data, self.datahub.data, self.trainer, **self.config)
            self.model.evaluate(self.trainer, self.load_model, self.extract_feature)
