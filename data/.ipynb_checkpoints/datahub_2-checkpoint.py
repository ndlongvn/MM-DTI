...
class DataHub_2(object):
    ...
    def __init__(self, data=None, is_train=True, save_path=None, **params):
        ...
        self.cache_dir_train_1 = params.get('cache_dir_train_1', None)
        self.cache_dir_train_2 = params.get('cache_dir_train_2', None)
        ...
        self.aug_smiles= params.get('aug_smiles', False)
    ...
    def _init_data(self, **params):
        ...
        """
        if self.is_train and self.aug_smiles:
            smile_52= pd.read_csv('/workspace1/longnd38/unimol/Uni-Mol/unimol_tools/data/train_split_randomized.csv')['smiles_seed_52'].values
            smile_62= pd.read_csv('/workspace1/longnd38/unimol/Uni-Mol/unimol_tools/data/train_split_randomized.csv')['smiles_seed_62'].values
        ...
        if self.augment and self.is_train:
            cache_dir_1= self.cache_dir_train_1
            cache_dir_2= self.cache_dir_train_2
            dict_params= copy.deepcopy(params)
            for cache_dir in [cache_dir_1, cache_dir_2]:
                if cache_dir is None:
                    continue
                if cache_dir is not None:
                    if os.path.exists(cache_dir):
                        with open(cache_dir, 'rb') as f:
                            no_h_list_aug = pickle.load(f)
                            logger.info("Load data aug from cache...")
                    
                    else:
                        if 'atoms' in self.data and 'coordinates' in self.data:
                            dict_params['seed']= dict_params['seed'] + 10
                            no_h_list_aug = ConformerGen(**dict_params).transform_raw(self.data['atoms'], self.data['coordinates'])
                        else:
                            dict_params['seed']= dict_params['seed'] + 10
                            smiles_list = self.data["smiles"]
                            no_h_list_aug = ConformerGen(**dict_params).transform(smiles_list)

                        with open(cache_dir, 'wb') as f:
                            pickle.dump(no_h_list_aug, f)
                            logger.info("Save data aug to cache...")
                            
                    for idx in range(len(no_h_list)):
                        for name in ['src_tokens', 'src_distance', 'src_coord', 'src_edge_type']:
                            if cache_dir == cache_dir_1:
                                no_h_list[idx][name + "_aug_1" ]= no_h_list_aug[idx][name]
                            else:
                                no_h_list[idx][name + "_aug_2" ]= no_h_list_aug[idx][name]
                    if self.aug_smiles:
                        for idx in range(len(no_h_list)):
                            if cache_dir == cache_dir_1:
                                no_h_list[idx]['smile_aug_1'] = smile_52[idx]
                            else:
                                no_h_list[idx]['smile_aug_2'] = smile_62[idx]
                    self.data['unimol_input'] = no_h_list
                else:
                    if 'atoms' in self.data and 'coordinates' in self.data:
                        dict_params['seed']= dict_params['seed'] + 10
                        no_h_list = ConformerGen(**dict_params).transform_raw(self.data['atoms'], self.data['coordinates'])
                    else:
                        dict_params['seed']= dict_params['seed'] + 10
                        smiles_list = self.data["smiles"]
                        no_h_list = ConformerGen(**dict_params).transform(smiles_list)
