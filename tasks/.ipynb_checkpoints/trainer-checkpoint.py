...
class Trainer(object):
    ...
    def _init_trainer(self, **params):
        ...
        self.freeze_model= params.get('freeze_model', False)
        ...
        self.augment_loss= params.get('augment_loss', False)
        self.num_aug= params.get('num_aug', 0)
    ...
    def fit_predict(self, model, train_dataset, valid_dataset, loss_func, activation_fn, dump_dir, fold, target_scaler, feature_name=None, return_ct_loss= False, return_rnc_loss= False, use_weight= False):
        """
        ...
        for epoch in range(self.max_epochs):
            ...
            for i, batch in enumerate(train_dataloader):
                ...
                if self.scaler and self.device.type == 'cuda':
                    with torch.cuda.amp.autocast( dtype=torch.float16):
                        if not return_ct_loss and not return_rnc_loss:
                            outputs = model(**net_input, epoch= epoch)
                            loss = self.alpha*loss_func(outputs, net_target)
                            regress_loss.append(float(loss_func(outputs, net_target).data))
                        else:
                            if return_rnc_loss and return_ct_loss:
                                if self.augment and self.num_aug!=0:
                                            # src_tokens,src_distance,src_edge_type,input_ids,attention_mask,weights
                                    
                                    
                                    outputs, ct_loss, rnc_loss = model(**net_input, return_ct_loss= return_ct_loss, return_rnc_loss= return_rnc_loss, net_target= net_target, use_weight= use_weight, epoch= epoch)
                                    # -----           
                                    # new
                                    src_tokens_aug= net_input['src_tokens_aug']
                                    src_distance_aug= net_input['src_distance_aug']
                                    src_edge_type_aug= net_input['src_edge_type_aug']
                                    input_ids_aug= net_input['input_ids_aug']
                                    attention_mask_aug= net_input['attention_mask_aug'] 
                                    net_target_aug = torch.cat([net_target for k in range(1+self.num_aug)], dim=0)
                                    # weights_aug= torch.cat([net_input['weights']  for k in range(1+self.num_aug)], dim=0)
                                    weights_aug= net_input['weights_aug'] 
                       
                                    
                                    
                                    
                                    outputs_aug, ct_loss_aug, rnc_loss_aug = model(src_tokens= src_tokens_aug,src_distance= src_distance_aug,src_edge_type= src_edge_type_aug,
                                                                       input_ids= input_ids_aug,attention_mask= attention_mask_aug,weights= weights_aug, reg= False,
                                                                        return_ct_loss= return_ct_loss, return_rnc_loss= return_rnc_loss, net_target= net_target_aug, use_weight= use_weight,
                                                                          epoch= epoch, train= False)
                                    if self.augment_loss:
                                        loss = self.alpha*loss_func(outputs, net_target) + self.beta*(ct_loss+ct_loss_aug) + self.beta*rnc_loss_aug + self.beta*rnc_loss
                                    else:
                                        loss = self.alpha*loss_func(outputs, net_target) + self.beta*ct_loss + self.beta*rnc_loss_aug + self.beta*rnc_loss
                                        # loss = self.alpha*loss_func(outputs_aug, net_target_aug) + self.beta*ct_loss_aug + self.beta*rnc_loss_aug 
                                    if self.augment_loss:
                                        # add data to list
                                        trn_ct_loss.append(float(ct_loss.data+ct_loss_aug.data))
                                        trn_rnc_loss.append(float(rnc_loss_aug.data+rnc_loss.data))
                                    else:
                                        # add data to list
                                        trn_ct_loss.append(float(ct_loss.data))
                                        trn_rnc_loss.append(float(rnc_loss_aug.data+rnc_loss.data))
                                else:
                                    # outputs, ct_loss, rnc_loss = model(**net_input, return_ct_loss= return_ct_loss, return_rnc_loss= return_rnc_loss, net_target= net_target, use_weight= use_weight, epoch= epoch)
                                    ...
                                    #                                     # add data to list
                                    # trn_ct_loss.append(float(ct_loss.data))
                                    # trn_rnc_loss.append(float(rnc_loss.data))
                                    outputs, ct_loss, rnc_loss = model(**net_input, return_ct_loss= return_ct_loss, return_rnc_loss= return_rnc_loss, net_target= net_target, use_weight= use_weight, epoch= epoch)
                                    ...
                                    trn_ct_loss.append(float(ct_loss.data))
                                    trn_rnc_loss.append(float(rnc_loss.data))
                                    regress_loss.append(float(rg_loss.data))
                            elif not return_rnc_loss and return_ct_loss:
                                outputs, ct_loss = model(**net_input, return_ct_loss= return_ct_loss, epoch= epoch, net_target= net_target, )
                                loss = self.alpha*loss_func(outputs, net_target) + self.beta*ct_loss
                                # add data to list
                                trn_ct_loss.append(float(ct_loss.data))
                                regress_loss.append(float(loss_func(outputs, net_target).data))
                            elif return_rnc_loss and not return_ct_loss:
                                outputs, rnc_loss = model(**net_input, return_rnc_loss= return_rnc_loss, net_target= net_target, use_weight= use_weight, epoch= epoch)
                    ...
                    with torch.set_grad_enabled(True):
                        if not return_ct_loss and not return_rnc_loss:
                            ...
                            if return_rnc_loss and return_ct_loss:
                                ...
                                outputs, ct_loss = model(**net_input, return_ct_loss= return_ct_loss, epoch= epoch)
                                loss = self.alpha*loss_func(outputs, net_target) + self.beta*ct_loss
                                # add data to list
                                trn_ct_loss.append(float(ct_loss.data))
                                regress_loss.append(float(loss_func(outputs, net_target).data))
                ...
                elif return_ct_loss and return_rnc_loss:
                        batch_bar.set_postfix(
                            Epoch="Epoch {}/{}".format(epoch+1, self.max_epochs),
                            loss="{:.04f}".format(float(sum(trn_loss) / (i + 1))),
                            ct_loss="{:.04f}".format(float(sum(trn_ct_loss) / (i + 1))),
                            rnc_loss="{:.04f}".format(float(sum(trn_rnc_loss) / (i + 1))),
                            lr="{:.04f}".format(float(optimizer.param_groups[0]['lr'])))
                elif return_ct_loss and not return_rnc_loss:
                    batch_bar.set_postfix(
                        Epoch="Epoch {}/{}".format(epoch+1, self.max_epochs),
                        loss="{:.04f}".format(float(sum(trn_loss) / (i + 1))),
                        ct_loss="{:.04f}".format(float(sum(trn_ct_loss) / (i + 1))),
            ...
            y_preds, val_loss, metric_score = self.predict(
                model, valid_dataset, loss_func, activation_fn, dump_dir, fold, target_scaler, epoch, load_model=False, feature_name=feature_name, return_ct_loss= False)
            end_time = time.time()
            total_val_loss = np.mean(val_loss)
            _score = list(metric_score.values())[0]
            _metric = list(metric_score.keys())[0]
            if return_ct_loss and return_rnc_loss:
                message= 'Epoch [{}/{}] train_loss: {:.4f}, train_rg_loss: {:.4f}, train_ct_loss: {:.4f}, train_rnc_loss: {:.4f}, val_loss: {:.4f}, val_{}: {:.4f}, lr: {:.6f}, {:.1f}s'.format(epoch+1, self.max_epochs, total_trn_loss, np.mean(regress_loss), np.mean(trn_ct_loss), np.mean(trn_rnc_loss), total_val_loss, _metric, _score, optimizer.param_groups[0]['lr'], (end_time - start_time))
            elif not return_ct_loss and return_rnc_loss:
                message= 'Epoch [{}/{}] train_loss: {:.4f}, train_rg_loss: {:.4f}, train_rnc_loss: {:.4f}, val_loss: {:.4f}, val_{}: {:.4f}, lr: {:.6f}, {:.1f}s'.format(epoch+1, self.max_epochs, total_trn_loss, np.mean(regress_loss), np.mean(trn_rnc_loss), total_val_loss, _metric, _score, optimizer.param_groups[0]['lr'], (end_time - start_time))
            else:
    ...
    def predict(self, model, dataset, loss_func, activation_fn, dump_dir, fold, target_scaler=None, epoch=1, load_model=False, feature_name=None, return_ct_loss= False, return_rnc_loss= False, return_feature= False):
        """
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.no_grad():
                if not return_ct_loss and not return_rnc_loss:
                    ...
                    if return_rnc_loss and return_ct_loss:
                        outputs, ct_loss, rnc_loss = model(**net_input, return_ct_loss= return_ct_loss, return_rnc_loss= return_rnc_loss, net_target= net_target)
                        loss = self.alpha*loss_func(outputs, net_target) + self.beta*ct_loss + self.beta*rnc_loss
                        # val_loss.append(float(loss.data))
                    elif not return_rnc_loss and return_ct_loss:
                        outputs, ct_loss = model(**net_input, return_ct_loss= return_ct_loss)
                        loss = self.alpha*loss_func(outputs, net_target) + self.beta*ct_loss
                        # val_loss.append(float(loss.data))
                    elif return_rnc_loss and not return_ct_loss:
                        outputs, rnc_loss = model(**net_input, return_rnc_loss= return_rnc_loss, net_target= net_target)
                        loss = self.alpha*loss_func(outputs, net_target) + self.beta*rnc_loss
