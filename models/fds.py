...
class FDS(nn.Module):
    ...
    def __init__(self, feature_dim, raw_data, col_data, using_scale, bucket_num=100, bucket_start=0, start_update=0, start_smooth=1,
                 kernel='gaussian', ks=5, sigma=2, momentum=0.9, device='cuda'):
        ...
        self.raw_data= pd.read_csv(raw_data).loc[:, col_data].values
        self.device = device

        # create bucket for set data
        regression_value= copy.deepcopy(self.raw_data)
        if using_scale:
            logger.info("Using StandardScaler to scale the regression values.")
            regression_value= StandardScaler().fit_transform(regression_value.reshape(-1, 1)).reshape(-1)
