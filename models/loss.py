...
class MaskedBCEWithLogitsLoss(nn.Module):
    def __init__(self):
        ...
        self.bce_loss = nn.BCEWithLogitsLoss(reduction='none')  # No reduction for masking
    
    def forward(self, logits, targets):
        # Replace NaN values with -1
        targets = torch.where(torch.isnan(targets), torch.full_like(targets, -1), targets)
        
        # Create a mask for targets that are either 0 or 1 (exclude others, including -1)
        mask = (targets == 0) | (targets == 1)
        
        # Compute the BCE loss without reduction
        loss = self.bce_loss(logits, targets)
        
        # Apply the mask to the loss
        loss = loss * mask
        
        # Return the mean loss, considering only the masked elements
...
def BCEwithNan(y_pred, y_true):
    """
    Calculates the Binary Cross-Entropy loss, ignoring NaN values in the target.

    :param y_pred: (torch.Tensor) Predicted values.
    :param y_true: (torch.Tensor) Ground truth values, may contain NaNs.
    
    :return: (torch.Tensor) BCE loss computed only on non-NaN elements.
    """
    mask = ~torch.isnan(y_true)
    y_pred = y_pred[mask]
    y_true = y_true[mask]
...
def FocalLossWithLogits(y_pred, y_true, alpha=0.25, gamma=2.0):
    ...
    mask = ~torch.isnan(y_true)
    def check_valid(y_true): 
        return ((y_true == 0.0) | (y_true == 1.0))
    mask = mask & check_valid(y_true)
    y_pred = y_pred[mask]
    y_true = y_true[mask]
