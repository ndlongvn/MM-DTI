...
class NNModel(object):
        ...
        if model_name in NNMODEL_REGISTER:
            ...
            if isinstance(freeze_module, list):
                ...
                for module_name in freeze_module:
                    encoder = getattr(model, module_name)
    ...
    def run(self):
        ...
        for fold, (tr_idx, te_idx) in enumerate(self.splitter.split(X, y, group)):
            ...
            _y_pred = self.trainer.fit_predict(
                self.model, traindataset, validdataset, self.loss_func, self.activation_fn, self.save_path, fold, self.target_scaler, return_ct_loss= self.using_ct, return_rnc_loss=self.using_rnc)
    ...
    def evaluate(self, trainer=None,  checkpoints_path=None):
        ...
        testdataset = NNDataset(self.features, label=np.asarray(self.data['target']))
