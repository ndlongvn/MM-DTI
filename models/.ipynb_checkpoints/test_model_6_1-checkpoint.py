...
from .fds import FDS
from .mag import pad_embedd, pad_mask, MAG_1
import argparse
import pathlib
import os
...
def crossmodal_config():
    ...
    args.beta_shift = getattr(args, "beta_shift", 1e-3)
    return args
class TestModel_6_1(nn.Module):
    """
    ...
    def __init__(self, output_dim=2, **params):
        ...
        self.fuse_type = 'mean'
        self.mag= MAG_1(self.cross_cfg.hidden_size, self.cross_cfg.beta_shift, self.cross_cfg.hidden_dropout_prob)
    ...
    def forward(
        ...
        **kwargs
        ...
        txt_out= main_addon_sequence_output
        vis_out= audio_output
        ...
        cross_output_layer = cross_encoder[-1] # mai



        cross_output_layer, cross_txt_output_layer= pad_embedd(cross_output_layer, cross_txt_output_layer)
        txt_out, vis_out= pad_embedd(txt_out, vis_out)

        output_embedd= self.mag(cross_output_layer, cross_txt_output_layer, txt_out, vis_out)

        # final_output = torch.cat((cross_txt_output_layer, cross_output_layer), dim=1)                                    

        if self.fuse_type=='mean':
            final_output = output_embedd
