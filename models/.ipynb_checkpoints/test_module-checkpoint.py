...
# import torch.nn.init as trunc_normal_
...
class InterCTCResModule(nn.Module):
    ...
    def __init__(self, dim_model, vocab_size):
        ...
        self.proj_1 = nn.Linear(dim_model, vocab_size)
        self.proj_2 = nn.Linear(vocab_size, dim_model)

    def forward(self, x):

        logits = self.proj_1(x)
        x = x + self.proj_2(logits)
...
class AgentAttention(nn.Module):
    ...
    def forward(self, s1_hidden_states, s2_hidden_states, s1_attention_mask= None, s2_attention_mask= None):
        ...
        if s2_attention_mask!=None:
            agent_attn= (agent_tokens * self.scale) @ k.transpose(-2, -1)+ s2_attention_mask
            agent_attn = self.softmax(agent_attn) #  + position_bias
        else:
        ...
        if s1_attention_mask!=None:
            ...
            q_attn = self.softmax((q_attn.transpose(-2, -1)+ s1_attention_mask).transpose(-2, -1)) # + agent_bias
        else:
