...
#         # proj_query = proj_x_l.mean(dim=1)
#         # proj_positive = proj_x_av.mean(dim=1)
...
#         self.info_proj_query = nn.Sequential(
#             nn.Linear(self.orig_d_l, self.orig_d_l),
#             nn.GELU(),
#             nn.Linear(self.orig_d_l, self.d_l)
#         )
#         self.info_proj_positive = nn.Sequential(
#             nn.Linear(self.orig_d_av, self.orig_d_av),
#             nn.GELU(),
#             nn.Linear(self.orig_d_av, self.d_l)
#         )
...
class InfoNCE(nn.Module):
    ...
    def __init__(self, bert_output_size, graph_ouput_size, temperature=0.1, reduction='mean', negative_mode='unpaired'):
        ...
        # self.info_proj_query = nn.Linear(self.orig_d_l, self.d_l)
        # self.info_proj_positive = nn.Linear(self.orig_d_av, self.d_av)
