...
class AgentAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        assert (config.hidden_size % config.num_attention_heads) == 0, 'wrong in modeling class 1.2.1.2'

        self.attention_size = int(config.hidden_size / config.num_attention_heads)
        self.all_head_size = config.hidden_size

        # 省去了最后多头和一的全连接层，可以改一下,参数变多，或许效果会好
    ...
    def transpose_for_head(self, embedding):
        new_shape = embedding.size()[:-1] + (self.num_head, self.attention_size)
        embedding = embedding.view(*new_shape)
        return embedding.permute(0, 2, 1, 3)

    def forward(self, word_embedding, image_embedding, attention_mask):

        word_query = self.query(word_embedding)
        image_key = self.key(image_embedding)
        image_value = self.value(image_embedding)

        head_query = self.transpose_for_head(word_query)
        head_key = self.transpose_for_head(image_key)
        head_value = self.transpose_for_head(image_value)

        scores = torch.matmul(head_query, head_key.transpose(-1, -2))
        scores = scores / math.sqrt(self.attention_size)
        
        probs = nn.Softmax(dim=-1)(scores)
        

        attention_mask_broadcasted = attention_mask.unsqueeze(1).expand(scores.size(0), 16, -1)  # Expand to [1, 16, 62]
        masked_indices = attention_mask_broadcasted.bool()  # Convert to boolean tensor
        probs.data[~masked_indices] = 0.0

        # This is actually dropping out entire tokens to attend to, which might
        # seem a bit unusual, but is taken from the original Transformer paper.
        probs = self.attention_dropout(probs)

        result_attention = torch.matmul(probs, head_value)
        result_attention = result_attention.permute(0, 2, 1, 3).contiguous()
        # , 表示 元组
        origin_shape = result_attention.size()[:-2] + (self.all_head_size,)
        result_attention = result_attention.view(*origin_shape)
...
class BertAgentAttention(nn.Module):
    ...
    def forward(self, s1_input_tensor, s2_input_tensor, s1_attention_mask=None):
        s1_cross_output = self.self(s1_input_tensor, s2_input_tensor, s1_attention_mask)
...
class BertAgentAttentionLayer(nn.Module):
    ...
    def forward(self, s1_hidden_states, s2_hidden_states, s1_attention_mask=None):
        attention_output = self.attention(s1_hidden_states, s2_hidden_states, s1_attention_mask)
...
class BertAgentEncoder(nn.Module):
    ...
    def forward(self, s1_hidden_states, s2_hidden_states, s1_attention_mask=None, output_all_encoded_layers=True):
        all_encoder_layers = []
        for layer_module in self.layer:
            s1_hidden_states = layer_module(s1_hidden_states, s2_hidden_states, s1_attention_mask)
