...
class FDS(nn.Module):
    ...
    def __init__(self, feature_dim, raw_data, bucket_num=100, bucket_start=0, start_update=0, start_smooth=1,
        ...
        regression_value_org= copy.deepcopy(self.raw_data)
        std= StandardScaler()
        regression_value= std.fit_transform(regression_value_org.reshape(-1, 1)).reshape(-1)
        regression_value= anomaly_clean_regression(regression_value_org)
        value_range = np.max(regression_value) - np.min(regression_value)
        bin_width = value_range / bucket_num
    ...
    def update_running_stats(self, features, labels, epoch):
        ...
        labels_0= self.std.inverse_transform(labels_0.reshape(-1, 1)).reshape(-1)
        label_bin= torch.Tensor([int((value-self.min_value)//self.bin_width) for value in labels_0]).cuda()
        # print("lab1:", label_bin)
