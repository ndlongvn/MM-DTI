...
class TestModel_6_class(nn.Module):
    def __init__(self, output_dim=2, **params):
        ...
        self.conr_w= params.get('conr_lamda', '')
        self.task= params.get('task', '')
        logger.info(f"Using conr lamda: {self.conr_w}")
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.task=="classification":
            from .rnc_class import ConR_single as ConR
        elif self.task=="multilabel_classification":       
            from .rnc_class import ConR_Multi as ConR
        ...
        if not return_feature:
            if return_ct_loss:
                ...
                if return_rnc_loss and net_target is not None: 
                    if use_weight:
                        rnc_loss= ConR(classification_feats_pooled, net_target, logits, weights= weights, lamda= self.conr_w)
                    else:
                        rnc_loss= ConR(classification_feats_pooled, net_target, logits, lamda= self.conr_w)
    ...
    def batch_collate_fn(self, samples):
        ...
        if 'smile' in samples[0][0].keys():
            batch_text = tokenizer([i[0]['smile'] for i in samples], padding=True, truncation= True, return_tensors="pt")
