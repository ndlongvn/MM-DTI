...
class TestModel_7(nn.Module):
    ...
    def __init__(self, output_dim=2, **params):
        ...
        self.final_attention= BertSelfEncoder(self.cross_cfg)
        self.fuse_type = 'mean'
    ...
    def forward(
        ...
        **kwargs
        ...
        final_mask = torch.cat((extended_img_mask, extended_txt_mask), dim=-1)

        final_output = self.final_attention(final_output, final_mask)[-1]

        if self.fuse_type=='mean':
            mask= torch.cat([~padding_mask, attention_mask], dim=1)
            # print(mask.shape)
            # print(final_output.shape)
            final_output[~mask]=0

            # classification_feats_pooled = torch.mean(final_output, dim=1)
            classification_feats_pooled = final_output.sum(dim=1)/(mask).sum(dim=1).view(-1, 1)
