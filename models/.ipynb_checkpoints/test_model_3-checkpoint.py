...
from ast import Not
from .test_module import BertSelfEncoder, BertCrossEncoder, BertConfig, InterCTCResModule
...
class TestModel_3(nn.Module):
    ...
    def __init__(self, output_dim=2, config=None,**params):
        ...
        super().__init__()
        data_type='molecule'
        config=  BertConfig('/media/data3/users/longnd/Uni-Mol/unimol_tools/unimol_tools/config/bert_cfg.json')
        self.args = molecule_architecture()
        self.output_dim = output_dim
        self.data_type = data_type
        self.remove_hs = params.get('remove_hs', False)
        ...
        self.bert= AutoModel.from_pretrained("DeepChem/ChemBERTa-77M-MLM")
        ...
        self.img2img_attention = BertCrossEncoder(config, 1)
        self.inter1 = InterCTCResModule(384, 18)
        self.inter2 = InterCTCResModule(384, 18)
    ...
    def forward(
        ...
        **kwargs
        ...
        converted_vis_embed_map_v2 = self.vismap2text(audio_output)
        ...
        """for graph"""
        # from 512 to 384
        converted_vis_embed_map = self.vismap2text_v2(audio_output)

        img_mask = padding_mask.clone() # .squeeze(1)
        ...
        cross_txt_output_layer, inter_1= self.inter1(cross_txt_output_layer) 
        cross_output_layer, inter_2= self.inter2(cross_output_layer)
        
        # print("cross_output_layer: ",cross_output_layer.shape)
            ### query: graph, key, value: text
        ...
        # print("cross_final_txt_layer: ",cross_final_txt_layer.shape)

        cross_final_img_encoder = self.img2img_attention(converted_vis_embed_map_v2, cross_output_layer, extended_txt_mask) 
        cross_final_img_layer = cross_final_img_encoder[-1]
        # print("cross_final_img_layer: ",cross_final_img_layer.shape)
        final_output = torch.cat((cross_final_txt_layer, cross_final_img_layer), dim=1)                                    
        ...
        #     classification_feats_pooled = final_output.mean(dim=1)
        logits = self.classification_head(classification_feats_pooled)

        if return_ct_loss:
            if not return_aux_loss:
                return logits, ct_loss
            else:
                return logits, ct_loss, inter_1, inter_2
        return logits
