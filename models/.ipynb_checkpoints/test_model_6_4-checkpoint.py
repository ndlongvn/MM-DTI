...
class TestModel_6_4(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        """change"""
        if train:
            bs= src_tokens.shape[0]//3
            assert bs*3==src_tokens.shape[0], "bs not match"
        # add bert
        ...
        if return_ct_loss:
            if train:
                ct_loss= self.infonce(all_repr[:bs, :, :], out_bert[:bs, :, :])+ self.infonce1(out_bert[:bs, :, :], all_repr[:bs, :, :])
            else:
        ...
        #             smoothed_features= classification_feats_pooled[:bs, :]
        ...
        if self.training and epoch >= self.fds_cfg.start_smooth and reg and self.use_fds:
            if train:
                smoothed_features = self.FDS.smooth(smoothed_features[:bs, :], net_target[:bs, :], epoch)
            else:
        ...
        if not return_feature:
            if return_ct_loss:
                ...
                if return_rnc_loss and net_target is not None: 
                    ...
                    if train:
                        return logits[:bs, :], ct_loss, rnc_loss
                    else:
