...
class TestModel_6_2(nn.Module):
    def __init__(self, output_dim=2, **params):
        ...
        self.attention_v1 = BertCrossEncoder(self.cross_cfg, 1)
        self.attention_v2 = BertCrossEncoder(self.cross_cfg, 1)
        ...
        self.orgin_linear_change = nn.Sequential(
            nn.Linear(self.cross_cfg.hidden_size, self.cross_cfg.hidden_size),
            ActivateFun('gelu'),
            nn.Linear(self.cross_cfg.hidden_size, self.cross_cfg.hidden_size)
        )
    ...
    def forward_encoder(
        ...
        **kwargs
        ...
        all_repr = self.dropout_audio_input(all_repr)
        ...
        main_addon_sequence_output_v2 = cross_output_layer
        # print("main_addon_sequence_output: ",main_addon_sequence_output.shape)

        converted_vis_embed_map_v2_v2 = cross_txt_output_layer #self.vismap2text(audio_output)

        converted_vis_embed_map_v = cross_txt_output_layer

        cross_txt_encoder_v2 = self.attention_v1(converted_vis_embed_map_v2_v2, main_addon_sequence_output_v2, extended_txt_mask)
        cross_txt_output_layer_v2 = cross_txt_encoder_v2[-1]

        cross_encoder_v2 = self.attention_v2(main_addon_sequence_output_v2, converted_vis_embed_map_v, extended_img_mask)
        cross_output_layer_v2 = cross_encoder_v2[-1]


        # final_output = torch.cat((cross_txt_output_layer, cross_output_layer), dim=1)                                    

        if self.fuse_type=='mean':
            cross_txt_output_layer_v2[padding_mask]=0.0
            cross_output_layer_v2[~attention_mask]=0.0
            final_output = torch.cat((cross_txt_output_layer_v2, cross_output_layer_v2), dim=1)
        ...
        if return_ct_loss:
            return classification_feats_pooled, ct_loss
    ...
    def forward(
        ...
        attention_mask,
        weights,
        src_tokens_aug_1=None,
        src_distance_aug_1=None,
        src_edge_type_aug_1=None,
        input_ids_aug_1=None,
        attention_mask_aug_1=None,
        return_ct_loss=False,
        ...
        **kwargs
        ...
        """
        if return_ct_loss:
            classification_feats_pooled, ct_loss= self.forward_encoder(src_tokens, src_distance, src_edge_type, input_ids, attention_mask, return_ct_loss=True)
        else:
            classification_feats_pooled= self.forward_encoder(src_tokens, src_distance, src_edge_type, input_ids, attention_mask, return_ct_loss=False)
        cl_loss= 0
        if self.training and src_distance_aug_1 is not None:

            classification_feats_pooled_aug= self.forward_encoder(src_tokens_aug_1, src_distance_aug_1, src_edge_type_aug_1, input_ids_aug_1, attention_mask_aug_1, return_ct_loss=False)

            orgin_res_change = self.orgin_linear_change(classification_feats_pooled)

            augment_res_change = self.augment_linear_change(classification_feats_pooled_aug)

            l_pos_neg = torch.einsum('nc,ck->nk', [orgin_res_change, augment_res_change.T])
            cl_lables = torch.arange(l_pos_neg.size(0), device=l_pos_neg.device)

            # if self.set_cuda:
            l_pos_neg /= 0.07

            cl_loss = F.cross_entropy(l_pos_neg, cl_lables)

        smoothed_features = classification_feats_pooled    
        if self.training and epoch >= self.fds_cfg.start_smooth and reg and self.use_fds:
        ...
        if self.training and src_distance_aug_1 is not None:
            logits_aug= self.classification_head(classification_feats_pooled_aug)
            
            classification_feats_pooled_c= torch.cat((classification_feats_pooled, classification_feats_pooled_aug), dim=0)
            net_target_c= torch.cat((net_target, net_target), dim=0)
            logits_c= torch.cat((logits, logits_aug), dim=0)
            ...
            if return_ct_loss:
                ...
                if return_rnc_loss and net_target is not None: 
                    if use_weight:
                        ...
                        if self.training and src_distance_aug_1 is not None:
                            rnc_loss+= ConR(classification_feats_pooled_c, net_target_c, logits_c, weights=weights_c)

                    else:
                        rnc_loss= ConR(classification_feats_pooled, net_target, logits)
                        if self.training and src_distance_aug_1 is not None:
                            rnc_loss+= ConR(classification_feats_pooled_c, net_target_c, logits_c)
                    if self.training and src_distance_aug_1 is not None:
                        return logits, ct_loss, rnc_loss, cl_loss
    ...
    def batch_collate_fn(self, samples):
        ...
        if 'smile_aug_1' in samples[0][0].keys():
            batch_text = tokenizer([i[0]['smile_aug_1'] for i in samples], padding=True, return_tensors="pt")
            batch_text_inputids = batch_text['input_ids']
            batch_text_attention = batch_text['attention_mask']
