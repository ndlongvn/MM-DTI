...
def fds_config(): 
    ...
    args.raw_data = getattr(args, "raw_data","data_regress/conan/freesolv/train.csv")
    return args  
...
class CrossAttentionModel(nn.Module):
    def __init__(self, cross_cfg, num_layers=1):
        ...
        # self.text_layernorm = nn.LayerNorm(cross_cfg.hidden_size)
        # self.graph_layernorm = nn.LayerNorm(cross_cfg.hidden_size)
...
class GatedFusionModel(nn.Module):
    def __init__(self, embed_dim):
        ...
        self.gate_layer = nn.Linear(embed_dim *2, embed_dim)  # Linear layer for gating
    ...
    def forward(self, text_embeddings, graph_embeddings):
        ...
        gate_values = self.sigmoid(self.gate_layer(combined_embeddings))  # Shape: [batch_size, seq_len, embed_dim]

        # Apply gating: element-wise multiplication
        gated_output = gate_values * text_embeddings + (1 - gate_values) * graph_embeddings
        return gated_output
...
class MM_Model(nn.Module):
    def __init__(self, output_dim=2, **params):
        ...
        self.conr_w = params.get('conr_w', 0.2)
        ...
        self.load_pretrained_weights(path="/workspace1/longnd38/unimol/Uni-Mol/unimol_tools_2/unimol_tools/weights/mol_pre_all_h_220816.pt")

        self.bert= AutoModel.from_pretrained("/workspace1/longnd38/unimol/MLM/MLP_TrainedModels")
        self.tokenizer= AutoTokenizer.from_pretrained("/workspace1/longnd38/unimol/MLM/MLP_TrainedModels")

        self.cross_modal_module= CrossAttentionModel(self.cross_cfg, num_layers=1)
        self.gated_fusion_module= GatedFusionModel(self.cross_cfg.hidden_size)
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.task == 'classification':
            ...
            from .contrastive import ConR_Single as ConR
        elif self.task == 'multilabel_classification':
            # from .conr_class import ConR_Multi as ConR
            from .contrastive import ConR_Multi as ConR
        elif self.task == 'regression':
            # from .conr import ConR
            from .contrastive import ConR_Regress as ConR
        # Prepare masks
        ...
        # Gated fusion
        classification_feats_pooled = self.gated_fusion_module(cross_txt_output_layer, cross_output_layer)
        # Fusion and pooling
        ...
        # if return_feature:
        #     if return_infonce_loss:
        #         if return_conr_loss and net_target is not None:
        #             rnc_loss = ConR(classification_feats_pooled, net_target, logits, weights=weights, w=self.conr_weight) if use_weight else ConR(classification_feats_pooled, net_target, logits, w=self.conr_weight)
...
class Chemberta(nn.Module):
    def __init__(self, model_name_or_path= "seyonec/ChemBERTa-zinc-base-v1", output_dim=18, **params):
    ...
    def forward(self, input_ids, attention_mask=None, **params):
    ...
    def batch_collate_fn(self, samples):
        batch= {}
        if 'smile' in samples[0][0].keys():
            batch_text = self.tokenizer([i[0]['smile'] for i in samples], padding=True, return_tensors="pt", truncation=True)
            batch_text_inputids = batch_text['input_ids']
            batch_text_attention = batch_text['attention_mask']
            batch['input_ids'] = batch_text_inputids
            batch['attention_mask'] = batch_text_attention
...
class ChembertaCustom(nn.Module):
    def __init__(self, model_name_or_path= 'seyonec/ChemBERTa-zinc-base-v1', num_labels=18, **params):
